...
export const TOOLS: Tool[] = [
  ...
  },
  {
    ...
    overview:
      'A JSON comparator you can edit inside. The two documents sit side by side as live editors with the difference drawn between them — red marks what only JSON 1 has, green what only JSON 2 has, and the arrows in the seam copy a block either way, so you can reconcile the pair as you read it. Neither side is the baseline. And because the verdict is structural rather than textual, reordered keys and different indentation are never counted as changes.',
    faqs: [
      {
